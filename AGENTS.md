...
SSH docs server that serves 51 documentation sources (docs + API specs) as a searchable markdown filesystem over SSH. The Node.js codebase fetches, normalises, and writes docs; a Docker image serves them via OpenSSH with `ForceCommand` routing.
...
pnpm fetch-docs           # fetch all 51 doc sources into ./docs/ (network-heavy, slow)
...
src/
  ...
  application/          — UpdateDocSets orchestrator, sources.ts (all 51 source definitions)
