/**
 * Canonical TypeScript template for the Pi extension file.
 *
 * Pi is a coding agent harness (github.com/earendil-works/pi-coding-agent).
 * Extensions use TypeBox types and the `defineTool` / `ExtensionAPI` API
 * from `@earendil-works/pi-coding-agent`, not Zod.
 *
 ...
 *
 * Install:
 *   ssh -p 2222 docs@HOST tools pi > ~/.pi/agent/extensions/docs.ts
 *   ssh -p 2222 docs@HOST agents pi >> ~/.pi/agent/AGENTS.md
 *
 * Or project-scoped:
 *   ssh -p 2222 docs@HOST tools pi > .pi/extensions/docs.ts
 */

// ─── Dynamic constants (injected by shell at runtime) ──────────────

export const PI_DYNAMIC_HEADER = `\
...
export const PI_EXPECTED_TOOLS = [
  "docs_search",
  "docs_read",
  "docs_find",
  "docs_grep",
